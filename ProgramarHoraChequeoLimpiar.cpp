...
void setup() {
  ...
  prefsCheck.begin("alarmaCheck", false); // Namespace "rtc"
  prefsClean.begin("alarmaClean", false); // Namespace "rtc"
  ...
  prefsCheck.begin("alarmCheck", false);
  prefsClean.begin("alarmClean", false);
...
void loop() {
  ...
  if (now.hour() == alarmCheckHour && now.minute() == alarmCheckMinute && now.second() == 0){
